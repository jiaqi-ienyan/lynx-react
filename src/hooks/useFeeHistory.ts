...
export const getBlockNumber = async (provider: BaseProvider): Promise<number> => {
...
export const combineHistory = (...histories: FeeHistory[]): FeeHistory => {
  histories.forEach((currentHistory, index) => {
    ...
    invariant(
      currentHistory.oldestBlock === previousHistory.oldestBlock + previousHistory.baseFeePerGas.length - 1,
  ...
  const baseFeePerGas = histories.flatMap(({ baseFeePerGas }) => baseFeePerGas.slice(0, -1)).concat(lastFeePerGas)
...
export const trimHistory = (history: FeeHistory, blocks: number): FeeHistory => {
...
export const useFeeHistory = <P extends JsonRpcProvider, W extends Web3Provider>(props?: {
  shouldFetch?: boolean
  providerRpc?: P
  providerWeb3?: W
  blocks?: number
  ...
  const result = useLidoSWR<FeeHistory, Error>(
    ...
    async (providerRpc: P, chainId: CHAINS, blocks: number): Promise<FeeHistory> => {
      ...
      const newestCachedBlock = blocksInCache ? oldestCachedBlock + blocksInCache - 1 : -1
      ...
      const shouldCombine = blocksInCache ? newestCachedBlock < newHistory.oldestBlock : false

      const combinedHistory = shouldCombine ? combineHistory(cachedHistory, newHistory) : newHistory
